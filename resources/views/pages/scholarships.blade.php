    ...
    <section class="py-24 bg-gradient-to-b from-slate-50 via-white to-slate-50 relative -mt-10 rounded-t-[40px] z-20 shadow-2xl border-t border-gray-100">
        <div class="container mx-auto px-6 max-w-5xl">
            ...
            @if(isset($scholarships) && $scholarships->count() > 0)
                <div class="space-y-10">
                    @foreach($scholarships as $index => $scholarship)
                        @php
                            $theme = $scholarship->color_theme ?: 'yellow';
                            $badgeStyles = [
                            ...
                            $iconBoxStyles = [
                                'yellow' => 'bg-amber-50 text-amber-500 border-amber-200/80',
                                'red' => 'bg-rose-50 text-red-600 border-red-200/80',
                            ...
                            ][$theme] ?? 'bg-amber-50 text-amber-500 border-amber-200/80';

                            $hoverText = [
                                'yellow' => 'group-hover:text-amber-600',
                                'red' => 'group-hover:text-red-600',
                                'blue' => 'group-hover:text-blue-600',
                                'emerald' => 'group-hover:text-emerald-600',
                                'purple' => 'group-hover:text-purple-600',
                            ][$theme] ?? 'group-hover:text-amber-600';
                        @endphp

                        <div class="group bg-white p-8 md:p-10 rounded-3xl shadow-xl hover:shadow-2xl border border-gray-100 hover:border-yellow-300/80 transition-all duration-300 flex flex-col md:flex-row gap-8 items-center relative overflow-hidden">
                            ...
                            <div class="w-24 h-24 md:w-28 md:h-28 {{ $iconBoxStyles }} rounded-2xl flex items-center justify-center text-4xl md:text-5xl shrink-0 shadow-inner group-hover:scale-105 group-hover:rotate-3 transition-transform duration-300 border">
                                <i class="fa-solid {{ $scholarship->icon ?: 'fa-award' }}"></i>
                            </div>
                            <div class="w-full text-left">
                                <div class="flex flex-wrap items-center gap-3 mb-3">
                                    @if($scholarship->badge)
                                        <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border {{ $badgeStyles }}">{{ $scholarship->badge }}</span>
                                    @endif
                                    @if($scholarship->highlight)
                                        <span class="text-xs font-semibold text-gray-500 flex items-center gap-1">{{ $scholarship->highlight }}</span>
                                    @endif
                                </div>
                                <h2 class="text-2xl md:text-3xl font-extrabold text-[#0b1b3d] mb-3 {{ $hoverText }} transition-colors">{{ $scholarship->title }}</h2>
                                <div class="text-gray-600 leading-relaxed mb-6 prose max-w-none text-base font-normal">{!! $scholarship->description !!}</div>
                                <div class="flex items-center gap-4">
                                    <a href="{{ $scholarship->button_link ?: route('contact') }}" class="inline-flex items-center gap-2 bg-[#0b1b3d] text-white px-7 py-3.5 rounded-xl font-bold text-sm shadow-md shadow-blue-900/20 hover:bg-yellow-500 hover:text-[#0b1b3d] hover:shadow-lg transition-all duration-200">
                                        <span>{{ $scholarship->button_text ?: 'Check Your Eligibility' }}</span>
            ...
            @else
                ...
                <!-- Card 2: Government & Regional Fallback -->

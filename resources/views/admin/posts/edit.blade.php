<x-app-layout>
    ...
    <div class="py-10 bg-slate-50 min-h-screen">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" enctype="multipart/form-data" id="post-edit-form" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="bg-white rounded-3xl shadow-xl border border-slate-100 p-8 md:p-10 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                        ...
                        <div>
                            <label class="block text-sm font-extrabold text-slate-800 mb-2">Hero Banner Image</label>
                            <div class="relative border-2 border-dashed border-slate-200 rounded-2xl p-4 bg-slate-50 hover:bg-slate-100/60 transition-colors text-center cursor-pointer" onclick="document.getElementById('post_image_input').click()">
                                ...
                                @if($post->image)
                                    <div id="image_preview_box" class="relative py-1">
                                        <img id="image_preview_img" src="{{ asset($post->image) }}" class="h-28 mx-auto rounded-xl object-cover shadow-md border">
                                        <p class="text-[11px] text-indigo-600 font-bold mt-2 flex items-center justify-center gap-1">
                                            <i class="fa-solid fa-camera"></i> Current Image (Click box to replace)
                                        </p>
                                    </div>
                                    <div id="image_placeholder" class="hidden py-4">
                    ...
                    <div class="pt-4">
                        ...
                        <textarea name="content" id="hidden_content_textarea" class="hidden" required>{{ old('content', $post->content) }}</textarea>
                    </div>
                    ...
                    <div class="pt-6 border-t border-slate-100 flex items-center justify-end gap-4">
                        ...
                        <button type="submit" onclick="syncQuillContent()" class="bg-gradient-to-r from-[#0b1b3d] to-blue-800 hover:from-blue-900 hover:to-[#0b1b3d] text-white font-extrabold py-3.5 px-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-2 transform hover:-translate-y-0.5 text-sm">
                            <i class="fa-solid fa-check"></i> Update Blog Post
                        </button>
                    </div>
